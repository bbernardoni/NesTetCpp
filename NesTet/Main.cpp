...
int main()
{
	...
	//int gh[] = {1,1,1,1,2,1,1,1,1,1,1}; solveGame(10, T, L, gh, 11);
	//int gh[] = {1,2,2,1,1,1,1,1,1,1}; solveGame(13, Z, I, gh, 10);
...
void solveGame(int seedIndex, lt cur, lt next, int gh[], int ghLen) {
	...
	//Sleep(10000);
#endif
	AI ai(emu, gh, ghLen, (250-counter)/4-10);
	ai.lineClear();
	ai.start();
}
...
void displayGame(int seedIndex, lt cur, lt next) {
	int endRand = Emu::generateSeed(wellMem, seedArray[seedIndex]);
	gui = new GUI(wellMem, cur, next, endRand, 50+seedIndex);
}
	
void displayEmpty(int rand, int startFrame, lt cur, lt next) {
	gui = new GUI(wellMem, cur, next, rand, startFrame);
}
